...
open class ResponseHandler {
    ...
    fun <T : Any> handleException(e: Exception): Resource<T> {
        return when (e) {
            is HttpException -> Resource.Error(getErrorMessage(e.code()), -5)
            ...
            else -> Resource.Error(getErrorMessage(Int.MAX_VALUE), -1)
        }
    }

    fun <T : Any> handleException(message: String): Resource<T> {
        Log.d("Error", "Message$message")
        Log.d("Error", "FormattedMessage${message}")
        return Resource.Error(message, -1)
    }

    fun <T : Any> handleException(statusCode: Int): Resource<T> {
        return Resource.Error(getErrorMessage(statusCode), -1)
    }
}
